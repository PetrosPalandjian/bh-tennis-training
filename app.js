...
function StretchTimer({
  ...
  const pickVoice = React.useCallback(() => {
    ...
    const preferred = [
      /daniel/i, /david/i, /alex/i, /thomas/i, /matthew/i,
      /male/i, /google us english/i, /en-us/i
    ];
    for (let i = 0; i < preferred.length; i++) {
      const v = voices.find(x => preferred[i].test(`${x.name} ${x.lang}`));
      if (v) return v;
    }
    return voices[0];
  }, []);
  ...
  React.useEffect(() => {
    ...
    if ((phase === "stretch" || phase === "rest") && cueKey !== prevKey) {
      if (phase === "stretch" && idx >= 0 && safeList[idx]) {
        speak(`Start! ${safeList[idx].name}. Let's go.`, true);
        announcedCue = true;
      } else if (phase === "rest") {
        const nextName = (idx + 1 < count && safeList[idx + 1]) ? safeList[idx + 1].name : "";
        speak(nextName ? `Rest. Breathe. Get ready for ${nextName}.` : "Rest. Breathe.", true);
        announcedCue = true;
      }
    ...
    if (!announcedCue && phase === "stretch" && timeLeft <= 5 && timeLeft >= 1) {
...
function App() {
  ...
  const isDynamicStretchTab = tab === "dynamic-stretches";
  const activeStretchType = isDynamicStretchTab ? "Dynamic" : "Static";
  const activeCatalogStretches = STRETCHES.filter(s => s.type === activeStretchType);
  ...
  const activeStretchList = isDynamicStretchTab ? dynamicList : staticList;
  const activeStretchKind = isDynamicStretchTab ? "dyn" : "stat";
  const activeStretchTitle = isDynamicStretchTab ? "Dynamic Stretches" : "Static Stretches";
  const activeStretchPhaseTitle = isDynamicStretchTab ? "Pre-Practice" : "After Practice";
  const activeStretchColor = isDynamicStretchTab ? BH.navy : BH.maroon;
  ...
  const updateStretchRow = async (patch) => {
    ...
    const { data, error } = await sb
      .from("stretch_session")
      .update(patch)
      .eq("id", 1)
      .select("*")
      .maybeSingle();
    if (error || !data) {
      const msg = error && error.message ? error.message : "No row updated";
      setToast(`Stretch timer update failed: ${msg}`);
      setTimeout(() => setToast(""), 4000);
      return;
    }
    setStretchSession(data);
  };
  ...
  const updateDrillRow = async (patch) => {
    ...
    if (error || !data) {
      const msg = error && error.message ? error.message : "No row updated";
      setToast(`Drill timer update failed: ${msg}`);
      setTimeout(() => setToast(""), 4000);
      return;
    }
    setDrillSession(data);
  };
  ...
  return (
    <div style={{minHeight:"100vh", background:BH.offWhite}}>
      ...
      <div style={{display:"flex", justifyContent:"center", gap:isSmall ? "8px" : "12px", padding:isSmall ? "10px 8px" : "16px",
        ...
        <button onClick={() => { setTab("dynamic-stretches"); }} style={tabBtn(tab==="dynamic-stretches")}>
          Dynamic Stretches
        </button>
        <button onClick={() => { setTab("static-stretches"); }} style={tabBtn(tab==="static-stretches")}>
          Static Stretches
        </button>
      </div>
      ...
      <div style={{maxWidth:"1200px", margin:"0 auto", padding:"20px"}}>
        ...
        {/* ===== STRETCH TABS ===== */}
        {(tab === "dynamic-stretches" || tab === "static-stretches") && (
          <div style={{display:"flex", gap:"20px", flexDirection:isMobile ? "column" : "row"}}>
            {admin && (
            <div style={{width:isMobile ? "100%" : "300px", flexShrink:0}}>
              <>
                  <div style={{background:BH.white, borderRadius:"8px", border:`1px solid ${BH.g300}`,
                    ...
                    <div style={{padding:"12px 14px", borderBottom:`1px solid ${BH.g200}`,
                                 fontSize:"13px", fontWeight:"bold", color:BH.navy}}>
                      All {activeStretchType} Stretches
                    </div>
                    {activeCatalogStretches.map(s => (
                  ...
                  <div style={{background:BH.white, borderRadius:"8px", border:`2px solid ${activeStretchColor}`,
                               marginTop:"12px", overflow:"hidden"}}>
                    <div style={{padding:"10px 14px", background:activeStretchColor, color:BH.white,
                                 fontSize:"13px", fontWeight:"bold"}}>
                      Today's {activeStretchTitle} ({activeStretchList.length})
                    </div>
                    <div style={{padding:"8px 14px", maxHeight:"140px", overflowY:"auto"}}>
                      {activeStretchList.length === 0 ? (
                        <div style={{fontSize:"12px", color:BH.g500, padding:"8px 0"}}>
                          No {activeStretchType.toLowerCase()} stretches selected.
                        </div>
                      ) : (
                        activeStretchList.map((s, i) => (
                          <div key={s.id} style={{display:"flex", gap:"8px", alignItems:"center", padding:"3px 0"}}>
                            <span style={{fontSize:"11px", fontWeight:"bold", color:activeStretchColor, minWidth:"18px"}}>{i+1}.</span>
            ...
            <div style={{flex:1, minWidth:0}}>
              {!admin && (
                <div style={{background:BH.white, borderRadius:"8px", border:`1px solid ${BH.g300}`, padding:"12px 14px", marginBottom:"10px"}}>
                  <div style={{fontSize:"12px", fontWeight:"bold", color:BH.navy, marginBottom:"6px"}}>Stretching Notes</div>
                  <div style={{fontSize:"12px", color:BH.g700, lineHeight:1.4}}>
                    Dynamic before hitting. Static after practice. Breathe and never force the range.
                  </div>
              ...
              {activeStretchList.length === 0 ? (
                <div style={{textAlign:"center", padding:"60px 20px", color:BH.g500}}>
                  {admin ? `No ${activeStretchType.toLowerCase()} stretches selected yet.` : "Waiting for coach..."}
                </div>
              ) : (
                <>
                  <div style={{display:"flex", gap:"10px", flexWrap:"wrap", marginBottom:"10px"}}>
                      <div style={{flex:1, minWidth:admin ? "220px" : "320px"}}>
                        <StretchTimer
                          label={isDynamicStretchTab ? "Dynamic Timer" : "Static Timer"}
                          color={activeStretchColor}
                          session={stretchReady ? stretchSession : null}
                          kind={activeStretchKind}
                          isAdmin={admin}
                          onAdminStart={() => adminStretchStart(activeStretchKind)}
                          onAdminPause={() => adminStretchPause(activeStretchKind)}
                          onAdminResume={() => adminStretchResume(activeStretchKind)}
                          onAdminReset={() => adminStretchReset(activeStretchKind)}
                          list={activeStretchList}
                          work={activeStretchWork}
                          rest={activeStretchRest}
                          onWorkChange={(e) => {
                            const v = Math.max(5, +e.target.value || 0);
                            if (isDynamicStretchTab) {
                              setDynStretchTime(v);
                  ...
                  <div style={{display:"grid", gridTemplateColumns:"1fr", gap:"14px"}}>
                    <div>
                      <div style={{fontSize:"13px", fontWeight:"bold", color:BH.navy, margin:"4px 0 10px"}}>{activeStretchPhaseTitle}</div>
                      <div style={{background:BH.white, borderRadius:"8px", border:`1px solid ${BH.g300}`}}>
                        {activeStretchList.length === 0 ? (
                        <div style={{padding:"16px", fontSize:"12px", color:BH.g500}}>No {activeStretchType.toLowerCase()} stretches selected.</div>
                        ) : (
                        activeStretchList.map((s, i) => {
                          const isActive = i === activeStretchProgress.idx && activeStretchProgress.phase === "stretch";
                          return (
                          <div key={s.id} style={{
                            padding:"10px 14px",
                            borderBottom:`1px solid ${BH.g100}`,
                            background: isActive
                              ? (isDynamicStretchTab ? "rgba(27,54,93,0.08)" : "rgba(122,28,42,0.08)")
                              : "transparent"
                          }}>
                            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:"10px"}}>
                              <div style={{fontSize:"13px", fontWeight:"bold", color:BH.navy}}>{i+1}. {s.name}</div>
                              <span style={{fontSize:"10px", color:BH.white, background:activeStretchColor, padding:"3px 8px",
                                borderRadius:"999px", fontWeight:"bold", letterSpacing:"0.3px"}}>{activeStretchType}</span>
                            </div>
